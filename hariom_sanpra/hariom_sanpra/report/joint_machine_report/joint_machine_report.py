...
def get_data(filters: frappe._dict) -> list[dict]:
	...
	rows = frappe.db.sql(
		...
		from `tabJoint Machine` se
		inner join `tabJoint Machine Items` sed on sed.parent = se.name
	...
	last_joint_machine_id = None
	for row in rows:
		current_joint_machine_id = row.get("joint_machine_id")
		if row.get("warehouse") != row.get("source_warehouse"):
			row["gsm"] = ""
		# row.pop("source_warehouse", None)
		if current_joint_machine_id== last_joint_machine_id:
			row["date"] = ""
			row["joint_machine_id"] = ""
			row["operator_name"] = ""
			row["machine_name"] = ""
			row["tag_in"] = ""
			row["tag_out"] = ""
			...
			last_joint_machine_id = current_joint_machine_id
