...
def get_columns() -> list[dict]:
	return [
		...
		{
			"label": _("Id"),
			"fieldname": "small_pipe_mc_id",
			"fieldtype": "Link",
			"options": "Small Pipe mc",
			"width": 140,
		},
		{
		...
		{
			"label": _("Thermocouple Chanages"),
			"fieldname": "thermocouple_chanages",
			"fieldtype": "Int",
			"width": 100,
		},
		{
			"label": _("Heater Change"),
			"fieldname": "heater_change",
			"fieldtype": "Int",
			"width": 100,
		},
		{
...
def get_data(filters: frappe._dict) -> list[dict]:
	...
	rows = frappe.db.sql(
		...
		select
			se.date,
			se.roll_size,
			se.thermocouple_chanages,
			se.heater_change,
			se.machine_speed,
			se.man_power,
			se.temperature,
			se.name as small_pipe_mc_id,
		...
		from `tabSmall Pipe mc` se
		left join `tabSmall Pipe Items` sed on sed.parent = se.name
	...
	last_small_pipe_mc_id = None
	for row in rows:
		current_small_pipe_mc_id = row.get("small_pipe_mc_id")
		if current_small_pipe_mc_id== last_small_pipe_mc_id:
			row["date"] = ""
			row["small_pipe_mc_id"] = ""
			row["operator_name"] = ""
			row["machine_name"] = ""
			row["tag_in"] = ""
			row["tag_out"] = ""
			...
			row["roll_size"] = ""
			row["heater_change"] = ""
			row["thermocouple_chanages"] = ""
			row["machine_speed"] = ""
			row["temperature"] = ""
		else:
			last_small_pipe_mc_id = current_small_pipe_mc_id
