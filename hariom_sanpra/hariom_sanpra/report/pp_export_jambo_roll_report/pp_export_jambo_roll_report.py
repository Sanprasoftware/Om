...
def get_columns() -> list[dict]:
	return [
		...
		{
			"label": _("Id"),
			"fieldname": "pp_export_jambo_roll_id",
			"fieldtype": "Link",
			"options": "PP EXport Jambo Roll",
			"width": 140,
		},
		{
...
def get_data(filters: frappe._dict) -> list[dict]:
	...
	last_pp_export_jambo_roll_id = None
	for row in rows:
		current_pp_export_jambo_roll_id = row.get("pp_export_jambo_roll_id")
		if current_pp_export_jambo_roll_id== last_pp_export_jambo_roll_id:
			row["date"] = ""
			row["pp_export_jambo_roll_id"] = ""
			row["operator_name"] = ""
			row["machine_name"] = ""
			row["tag_in"] = ""
			row["tag_out"] = ""
			...
			last_pp_export_jambo_roll_id = current_pp_export_jambo_roll_id
