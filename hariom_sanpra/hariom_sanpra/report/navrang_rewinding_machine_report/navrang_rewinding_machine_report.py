...
def get_columns() -> list[dict]:
	return [
		...
		{
			"label": _("Id"),
			"fieldname": "navrang_rewinding_machine_id",
			"fieldtype": "Link",
			"options": "Navrang Rewinding Machine",
			"width": 140,
		},
		{
		...
		{
			"label": _("Target Prod. Per Shift Rolls"),
			"fieldname": "target_prod_per_shift_rolls",
			"fieldtype": "Float",
			"width": 170,
		},
		{
			"label": _("Target Wastage % Per Shift"),
			"fieldname": "target_wastage__per_shift",
			"fieldtype": "Float",
			"width": 170,
		},
		{
...
def get_data(filters: frappe._dict) -> list[dict]:
	...
	rows = frappe.db.sql(
		...
		select
			se.date,
			se.man_power,
			se.name as navrang_rewinding_machine_id,
			ifnull(emp.employee_name, se.operator_name) as operator_name,
			se.machine_name as machine_name,
			se.tag_in,
			se.target_prod_per_shift_rolls,
			se.target_wastage__per_shift,
			se.tag_out,
		...
		from `tabNavrang Rewinding Machine` se
		inner join `tabnavrang rewinding items` sed on sed.parent = se.name
	...
	last_navrang_rewinding_machine_id = None
	for row in rows:
		current_navrang_rewinding_machine_id = row.get("navrang_rewinding_machine_id")
		if current_navrang_rewinding_machine_id == last_navrang_rewinding_machine_id:
			row["date"] = ""
			row["navrang_rewinding_machine_id"] = ""
			row["operator_name"] = ""
			row["machine_name"] = ""
			row["tag_in"] = ""
			row["tag_out"] = ""
			...
			row["man_power"] = ""
			row["uom"] = ""
			row["target_prod_per_shift_rolls"] = ""
			row["target_wastage__per_shift"] = ""
		else:
			last_navrang_rewinding_machine_id = current_navrang_rewinding_machine_id
