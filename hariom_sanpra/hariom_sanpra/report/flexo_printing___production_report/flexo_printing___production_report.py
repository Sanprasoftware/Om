...
def get_columns() -> list[dict]:
	return [
		...
		{
			"label": _("Id"),
			"fieldname": "flexo_printing_production_id",
			"fieldtype": "Link",
			"options": "Flexo Printing - Production",
			"width": 140,
		},
		{
		...
		{
			"label": _("NO . OF ROLLS"),
			"fieldname": "no__of_rolls",
			"fieldtype": "int",
			"width": 150
		},
		{
			"label": _("Target Prod. Per Shift Kgs"),
			"fieldname": "target_prod_per_shift_kgs",
			"fieldtype": "Float",
			"width": 150
		},
		{
			"label": _("100Actual Wastage %"),
			"fieldname": "actual_wastage_",
			"fieldtype": "Float",
			"width": 150
		},
		{
...
def get_data(filters: frappe._dict) -> list[dict]:
	...
	rows = frappe.db.sql(
		...
		select
			...
			se.no__of_rolls,
			se.actual_wastage_,
			se.target_prod_per_shift_kgs,
			se.name as flexo_printing_production_id,
			(
				select group_concat(
		...
		from `tabFlexo Printing - Production` se
		inner join `tabFlexo PP Item` sed on sed.parent = se.name
	...
	last_flexo_printing_production_id = None
	for row in rows:
		current_flexo_printing_production_id = row.get("flexo_printing_production_id")
		if current_flexo_printing_production_id== last_flexo_printing_production_id:
			row["date"] = ""
			row["flexo_printing_production_id"] = ""
			row["operator_name"] = ""
			row["machine_name"] = ""
			row["tag_in"] = ""
			row["tag_out"] = ""
			...
			row["man_power"] = ""
			row["no__of_rolls"] = ""
			row["target_prod_per_shift_kgs"] = ""
			row["actual_wastage_"] = ""
		else:
			last_flexo_printing_production_id = current_flexo_printing_production_id
